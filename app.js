...
let searchInput, tagFilter, deleteNoteBtn, logoutBtn, fab, closeNoteBtn, emptyState, syncIndicator;
...
async function backupToDrive() {
  try {
    ...
    toast("Backup failed ❌");
  } finally { syncIndicator.style.display = "none"; }
}
...
async function handleNoteSubmit(e) {
  ...
  if (noteFilesInput.files.length > 0) {
    ...
    } catch (err) { console.error(err); toast("File upload failed ❌"); }
  }
...
window.onload = async () => {
  ...
  Object.assign(syncIndicator.style, { position: "fixed", bottom: "20px", right: "20px", background: "rgba(0,0,0,0.7)", color: "#fff", padding: "10px 15px", borderRadius: "8px", fontSize: "14px", display: "none" });
  document.body.appendChild(syncIndicator);

  fab.addEventListener("click", openNewNoteDialog);
  document.querySelectorAll(".color-btn").forEach(btn => btn.addEventListener("click", () => { noteColor.value = btn.dataset.color; }));
