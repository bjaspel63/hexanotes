// ======== Config (Drive folder & file names) ========
...
// ===== Ensure gapi client initialized =====
async function ensureGapiReady() {
  if (window.gapi.client && gapi.client.drive) return;
...
async function restoreNotes(){
  try {
    ...
    if(!Array.isArray(data)) throw new Error("Backup corrupted.");
    notes=data; saveNotes(); renderNotes();
